    ...
    public partial class LmDataGridView : UserControl, ILmControl
    {
        ...
        public bool MostrarTotalizador
        {
            ...
            set
            {
                ...
                if (!mostrarTotalizador)
                {
                    ...
                    GridRodape.Visible = false;
                }
                else
                {
                    Grid.ScrollBars = ScrollBars.Vertical;
                    GridRodape.Visible = true;
                }
        ...
        private void Grid_ColumnWidthChanged(object sender, DataGridViewColumnEventArgs e)
        {
            try
            {
                if (MostrarTotalizador)
                {
                    GridRodape.Columns[e.Column.Index].Width = Grid.Columns[e.Column.Index].Width;
        ...
        private void DefinirTamanhoGrid_e_Rodape()
        {
            try
            {
                Invoke(new MethodInvoker(delegate ()
                {
                    if (Controles.HScrollAtivo(GridRodape))
                        GridRodape.Height = 18 + 17;
                    else
                        GridRodape.Height = 18;

                    //GridRodape.BackColor =
                    //    Grid.ColumnHeadersDefaultCellStyle.BackColor;
                }));
            }
            catch (Exception)
        ...
        private void CriarRotuloSomatorio<T>()
        {
            try
            {
                Invoke(new MethodInvoker(delegate ()
                {
                    if (MostrarTotalizador)
                    {
                        ...
                        {
                            DataGridViewColumn cNew = new DataGridViewColumn
                            {
                                Name = "rdp" + c.Name,
                            ...
                            cNew.HeaderCell.Style.Alignment = c.HeaderCell.Style.Alignment;

                            PropertyInfo pro = obj.GetType().GetProperty(c.Name);

                            var atbCalculoTotal = (CalcularTotal)pro.GetCustomAttribute(typeof(CalcularTotal));

                            if(atbCalculoTotal != null)
                            {

                            }

                            GridRodape.Columns.Add(cNew);
