...
{
    public partial class FrmCaixaMultiselecao : LmChildForm
    {
        MouseHook mh;
        ...
        private void FrmCaixaMultiselecao_Load(object sender, EventArgs e)
        {
            ...
            foreach (DataGridViewRow row in dgv.Rows)
            {
                if ((bool)row.Cells["Select"].Value == true)
                {
                    ...
                    row.DefaultCellStyle.BackColor = LmPaint.BackColor.GridView.CellSelected(this.dgv.Theme);
                }
            }
        }
        ...
        private void dgv_CellValueChanged(object sender, DataGridViewCellEventArgs e)
        {
            if (e.ColumnIndex == dgv.CurrentRow.Cells["Select"].ColumnIndex)
            {
                if ((bool)dgv.CurrentRow.Cells["Select"].Value == true)
                {
                    ...
                    dgv.CurrentRow.DefaultCellStyle.BackColor = LmPaint.BackColor.GridView.CellSelected(this.dgv.Theme);
                }
                else
                {
                    dgv.CurrentRow.DefaultCellStyle.SelectionBackColor =
                    dgv.CurrentRow.DefaultCellStyle.BackColor = LmPaint.BackColor.GridView.CellNormal(this.dgv.Theme);
                }
            }
        }
        ...
        private void CmsPrimeiro_Click(object sender, EventArgs e)
        {
            if (int.TryParse(cmsPrimeiro.Tag.ToString(), out int rowIndex))
            {
                ...
                dgv.Rows[rowIndex].DefaultCellStyle.SelectionBackColor =
                    dgv.Rows[rowIndex].DefaultCellStyle.BackColor = LmPaint.BackColor.GridView.CellSelected(this.dgv.Theme);
                ...
                for (int i = dgv.RowCount - 1; i >= 0; i--)
                {
                    if (i != rowIndex)
                    {
                        ...
                        dgv.Rows[i].DefaultCellStyle.SelectionBackColor =
                            dgv.Rows[i].DefaultCellStyle.BackColor = LmPaint.BackColor.GridView.CellNormal(this.dgv.Theme);
                    }
                }
                SelectedValuesChanged?.Invoke(this, new EventArgs());
            }
